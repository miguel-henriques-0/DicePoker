...
import pt.isel.daw.dicepoker.TestClock
...
import pt.isel.daw.dicepoker.repository.jdbi.JdbiTransactionManager
...
class UserServiceTests {
    ...
    fun `can create user, token, and retrieve by token`() {
        ...
        val inviteCode = getInvitationCode(userService, 1)

        // when: creating a user
        val username = newTestUserName()
        val password = "changeit"
        // then: the creation is successful
        when (val createUserResult = userService.createUser(username, password, inviteCode)) {
        ...
        // then: the creation is successful
        val token =
            when (val createTokenResult = userService.createToken(username, password)) {
    ...
    fun `can logout`() {
        ...
        val token =
            when (val tokenCreationResult = userService.createToken(username, password)) {
    ...
    fun getInvitationCode(
        ...
        return when (val inviteCodeResult = usersService.createInviteCode(userId)) {
            is Either.Left -> fail("Could not get user $userId code")
            is Either.Right -> inviteCodeResult.value.inviteCode
        }
    }
        ...
        private fun createUsersService(
            ...
            UsersDomain(
                ...
                UsersDomainConfig(
                    ...
                    maxInvitesPerUser = 10000,
                    // user creation is dictated by one-time user invite codes
                ),
            ),
            testClock,
        )
